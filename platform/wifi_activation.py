...
def client_up_tail(ctx: ActivationContext, adapter,
    ...
    if spec.clear_dead_adapter:
        ...
        # (recovered or handed to another radio): clear the RESET_USB and
        # reactivate-first spends so a fresh wedge gets its one budgeted reset
        # and reactivate attempt again.
        with ctx.state_lock:
            ctx.RECOVERY_STATE.failover_reset_done.clear()
            ctx.RECOVERY_STATE.wedged_reactivate_done.clear()
