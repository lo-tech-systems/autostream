...
class TestInstallerNetworkFileHandling:
    ...
    def test_saved_selection_has_single_profile_fallback(self):
        """The suitable_count==1 fallback (which does not require
        autoconnect=yes) must remain, so a lone profile — including one a user set
        to autoconnect=no before install — is still imported under the
        client-profiles-autoconnect=no migration."""
        start = INSTALL_SH.find("_select_saved_wifi_connection()")
        body = INSTALL_SH[start: start + 3500]
        assert "suitable_count == 1" in body, (
            "the single-suitable-profile fallback must remain so the "
            "autoconnect=no profiles are still selectable at install"
        )
        # The client-profiles-autoconnect=no trace conclusion is recorded at the
        # selection site (the note sits in the comment block directly above the
        # function).
        assert "Client-profiles-autoconnect=no trace" in INSTALL_SH
