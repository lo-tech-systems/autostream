...
import autostream_appliance_identity as identity
...
def _reset_appliance_id_cache():
    ...
    orig_cache = identity._appliance_id_cache
    orig_set = identity._appliance_id_cache_set
    orig_err = identity._appliance_id_error_logged
    yield
    with identity._ID_CACHE_LOCK:
        identity._appliance_id_cache = orig_cache
        identity._appliance_id_cache_set = orig_set
        identity._appliance_id_error_logged = orig_err


def _clear_cache():
    with identity._ID_CACHE_LOCK:
        identity._appliance_id_cache = None
        identity._appliance_id_cache_set = False
        identity._appliance_id_error_logged = False
...
class TestNamespacedIdHash:
    def test_returns_20_char_lowercase_hex(self):
        result = identity._namespaced_id_hash("ns:", "deadbeef1234")
    ...
    def test_deterministic_for_same_inputs(self):
        r1 = identity._namespaced_id_hash("autostream-appliance-v1:", "00000000abcdef12")
        r2 = identity._namespaced_id_hash("autostream-appliance-v1:", "00000000abcdef12")
        assert r1 == r2
    ...
    def test_known_hash_vector(self):
        ...
        assert identity._namespaced_id_hash(namespace, serial) == expected_full[:20]

    def test_normalizes_uppercase_serial(self):
        r_lower = identity._namespaced_id_hash("ns:", "deadbeef")
        r_upper = identity._namespaced_id_hash("ns:", "DEADBEEF")
        assert r_lower == r_upper

    def test_strips_whitespace_from_serial(self):
        r_plain = identity._namespaced_id_hash("ns:", "abcd1234")
        r_padded = identity._namespaced_id_hash("ns:", "  abcd1234  ")
        assert r_plain == r_padded

    def test_empty_serial_returns_none(self):
        assert identity._namespaced_id_hash("ns:", "") is None

    def test_whitespace_only_serial_returns_none(self):
        assert identity._namespaced_id_hash("ns:", "   ") is None

    def test_non_hex_serial_returns_none(self):
        assert identity._namespaced_id_hash("ns:", "xyz!") is None

    def test_serial_with_embedded_space_returns_none(self):
        # After strip, "ab cd ef" is not valid hex
        assert identity._namespaced_id_hash("ns:", "ab cd ef") is None

    def test_different_namespaces_produce_different_ids(self):
        r1 = identity._namespaced_id_hash("autostream-appliance-v1:", "deadbeef")
        r2 = identity._namespaced_id_hash("autostream-dial-v1:", "deadbeef")
        assert r1 != r2

    def test_raw_serial_not_in_output(self):
        serial = "12345678abcdef90"
        result = identity._namespaced_id_hash("autostream-appliance-v1:", serial)
        assert result is not None
...
class TestReadIdFile:
    def test_reads_valid_20char_hex_file(self, tmp_path):
        ...
        assert identity._read_id_file(f) == "deadbeef1234567890ab"
    ...
    def test_missing_file_returns_none(self, tmp_path):
        assert identity._read_id_file(tmp_path / "nonexistent") is None

    def test_wrong_length_returns_none(self, tmp_path):
        f = tmp_path / "appliance-id"
        f.write_text("deadbeef", encoding="utf-8")
        assert identity._read_id_file(f) is None
...
class TestGetApplianceIdPrimary:
    def test_returns_20char_lowercase_hex(self):
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value="00000000abcdef12"):
            result = identity.get_appliance_id()
        assert result is not None
        assert len(result) == 20
        assert result == result.lower()
        assert all(c in "0123456789abcdef" for c in result)
    ...
    def test_deterministic_for_same_serial(self):
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value="00000000abcdef12"):
            r1 = identity.get_appliance_id()
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value="00000000abcdef12"):
            r2 = identity.get_appliance_id()
        assert r1 == r2

    def test_matches_known_hash_vector(self):
        _clear_cache()
        serial = "00000000abcdef12"
        with patch.object(identity, "get_cpu_serial", return_value=serial):
            result = identity.get_appliance_id()
    ...
    def test_normalizes_uppercase_serial(self):
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value="AABBCCDD12345678"):
            result_upper = identity.get_appliance_id()
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value="aabbccdd12345678"):
            result_lower = identity.get_appliance_id()
    ...
    def test_appliance_namespace_differs_from_dial_namespace(self):
        ...
        _clear_cache()
        with patch.object(identity, "get_cpu_serial", return_value=serial):
            appliance_id = identity.get_appliance_id()
        dial_id_direct = identity._namespaced_id_hash(identity._DIAL_NAMESPACE, serial)
...
class TestGetApplianceIdFallback:
    def test_uses_fallback_file_when_serial_unavailable(self, tmp_path):
        ...
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "APPLIANCE_ID_FILE", fallback):
            result = identity.get_appliance_id()
    ...
    def test_invalid_serial_and_missing_fallback_returns_none(self, tmp_path):
        ...
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "APPLIANCE_ID_FILE", missing):
            result = identity.get_appliance_id()
...
class TestGetApplianceIdCache:
    def test_result_is_cached_for_process_lifetime(self):
        ...
        with patch.object(identity, "get_cpu_serial", side_effect=counting_serial):
            r1 = identity.get_appliance_id()
            r2 = identity.get_appliance_id()
            r3 = identity.get_appliance_id()
    ...
    def test_cache_is_thread_safe(self):
        ...
        with patch.object(identity, "get_cpu_serial", return_value="deadbeef12345678"):
            def worker():
                results.append(identity.get_appliance_id())
...
class TestGetApplianceIdErrorLogging:
    def test_logs_one_error_when_identity_unavailable(self, tmp_path, caplog):
        ...
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "APPLIANCE_ID_FILE", missing):
            import logging
            with caplog.at_level(logging.ERROR, logger="autostream_appliance_identity"):
                identity.get_appliance_id()
                identity.get_appliance_id()  # second call must not log again
...
def _clear_dial_cache():
    with identity._ID_CACHE_LOCK:
        identity._dial_id_cache = None
        identity._dial_id_cache_set = False
        identity._dial_id_error_logged = False


@pytest.fixture(autouse=True)
def _reset_dial_id_cache():
    """Reset dial identity cache between tests."""
    orig_cache = identity._dial_id_cache
    orig_set = identity._dial_id_cache_set
    orig_err = identity._dial_id_error_logged
    yield
    with identity._ID_CACHE_LOCK:
        identity._dial_id_cache = orig_cache
        identity._dial_id_cache_set = orig_set
        identity._dial_id_error_logged = orig_err
...
class TestGetDialId:
    ...
    def test_deterministic_for_same_serial(self):
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value="00000000abcdef12"):
            r1 = identity.get_dial_id()
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value="00000000abcdef12"):
            r2 = identity.get_dial_id()
        assert r1 == r2
    ...
    def test_distinct_from_appliance_id_for_same_serial(self):
        ...
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value=serial):
            appliance = identity.get_appliance_id()
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value=serial):
            dial = identity.get_dial_id()
        assert appliance != dial

    def test_normalizes_uppercase_serial(self):
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value="AABBCCDD12345678"):
            r_upper = identity.get_dial_id()
        _clear_dial_cache()
        with patch.object(identity, "get_cpu_serial", return_value="aabbccdd12345678"):
            r_lower = identity.get_dial_id()
    ...
    def test_uses_dial_fallback_file(self, tmp_path):
        ...
        fallback.write_text("cafebabe12345678ab01", encoding="utf-8")
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "DIAL_ID_FILE", fallback):
            result = identity.get_dial_id()
        assert result == "cafebabe12345678ab01"
    ...
    def test_dial_fallback_is_distinct_from_appliance_fallback(self, tmp_path):
        ...
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "APPLIANCE_ID_FILE", appliance_fb), \
             patch.object(identity, "DIAL_ID_FILE", dial_fb):
            app_id = identity.get_appliance_id()
            _clear_dial_cache()
            dial_id = identity.get_dial_id()
    ...
    def test_returns_none_when_both_unavailable(self, tmp_path):
        ...
        with patch.object(identity, "get_cpu_serial", return_value=""), \
             patch.object(identity, "DIAL_ID_FILE", missing):
            result = identity.get_dial_id()
        assert result is None
