...
def _write_meminfo(path, mem_total_kib, mem_available_kib, swap_total_kib=0, swap_free_kib=0):
    path.write_text(
...
class TestGetEffectiveMemoryInfo:
    def test_returns_raw_mem_available_no_swap_used(self, tmp_path):
        meminfo = tmp_path / "meminfo"
        _write_meminfo(meminfo, 1_000_000, 500_000, 200_000, 200_000)
        result = su.get_effective_memory_info(meminfo_path=meminfo)
        assert result == (500_000 // 1024, 1_000_000 // 1024)

    def test_returns_raw_mem_available_regardless_of_swap_used(self, tmp_path):
        # Swap heavily in use (swap_used=100_000 kB): the effective figure is
        # exactly MemAvailable, with no deduction -- this is the whole point
        # of the simplification (previously this would have been reduced).
        meminfo = tmp_path / "meminfo"
        _write_meminfo(meminfo, 1_000_000, 500_000, 200_000, 100_000)
        result = su.get_effective_memory_info(meminfo_path=meminfo)
        assert result == (500_000 // 1024, 1_000_000 // 1024)

    def test_swap_fields_absent_no_effect(self, tmp_path):
        meminfo = tmp_path / "meminfo"
        meminfo.write_text(
    ...
    def test_missing_meminfo_file_returns_none(self, tmp_path):
        result = su.get_effective_memory_info(meminfo_path=tmp_path / "missing")
        assert result is None

    def test_default_path_used_when_unset(self):
        # Sanity check that calling with no args still hits the real
        # /proc/meminfo (i.e. the default is wired through), without
