...
class TestSameL3Segment:
    def _patch_facts(self, addrs, gateways):
        return patch.multiple(
            wifi_net,
            list_interface_addresses=MagicMock(side_effect=lambda ifname=None: addrs),
            _default_gateways_ipv4=MagicMock(side_effect=lambda ifname: gateways.get(ifname, [])),
        )

    def test_same_network_and_gateway_returns_true(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24}],
        }
        gateways = {"wlan1": ["192.168.1.1"], "eth0": ["192.168.1.1"]}
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is True

    def test_same_network_different_gateway_returns_false(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24}],
        }
        gateways = {"wlan1": ["192.168.1.1"], "eth0": ["192.168.1.254"]}
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False

    def test_different_network_returns_false(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [{"family": "ipv4", "address": "192.168.2.30", "prefixlen": 24}],
        }
        gateways = {"wlan1": ["192.168.1.1"], "eth0": ["192.168.1.1"]}
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False

    def test_missing_ipv4_or_gateway_returns_false(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [],
        }
        ...
        addrs["eth0"] = [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24}]
        gateways = {"wlan1": ["192.168.1.1"], "eth0": []}
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False
    ...
    def test_invalid_prefix_returns_false(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 99}],
        }
        gateways = {"wlan1": ["192.168.1.1"], "eth0": ["192.168.1.1"]}
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False

    def test_multiple_ipv4s_on_either_interface_returns_false(self):
        addrs = {
            "wlan1": [
                {"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24},
                {"family": "ipv4", "address": "192.168.1.21", "prefixlen": 24},
            ],
            "eth0": [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24}],
        }
        gateways = {"wlan1": ["192.168.1.1"], "eth0": ["192.168.1.1"]}
        ...
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [
                {"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24},
                {"family": "ipv4", "address": "192.168.1.31", "prefixlen": 24},
            ],
        }
        with self._patch_facts(addrs, gateways):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False

    def test_multiple_gateways_on_either_interface_returns_false(self):
        addrs = {
            "wlan1": [{"family": "ipv4", "address": "192.168.1.20", "prefixlen": 24}],
            "eth0": [{"family": "ipv4", "address": "192.168.1.30", "prefixlen": 24}],
        }
        with self._patch_facts(addrs, {
            "wlan1": ["192.168.1.1", "192.168.1.254"],
            "eth0": ["192.168.1.1"],
        }):
            assert wifi_net.same_l3_segment("wlan1", "eth0") is False
